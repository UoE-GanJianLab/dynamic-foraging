...
if not isdir(figure_7_figure_root):
    mkdir(figure_7_figure_root)
panel_abc_figure_root = pjoin(figure_7_figure_root, 'panel_abc')
if not isdir(panel_abc_figure_root):
    mkdir(panel_abc_figure_root)
panel_abc_significant = pjoin(figure_7_figure_root, 'panel_abc', 'significant')
if not isdir(panel_abc_significant):
    mkdir(panel_abc_significant)


figure_7_data_root = pjoin('data', 'spike_times', 'figure_7')
figure_data_root = pjoin('figure_data', 'figure_7')
...
figure_7_selected = ['AKED0420210804_pfc_10_dms_23', 'AKED0520210726_pfc_16_dms_18', 'AKED0120210721_pfc_6_dms_0']
...
def figure_7_panel_abc(session_name: str, pfc_name: str, dms_name: str, pfc_times: np.ndarray, dms_times: np.ndarray, cue_times: np.ndarray, reward_proportion: np.ndarray, reset: bool = False, plot: bool = True):
    # load the interconnectivity strength if it exists
    if isfile(pjoin(figure_7_data_root, f'{session_name}_{pfc_name}_{dms_name}_interconnectivity_strength.npy')) and not reset:
        interconnectivity_strength = np.load(pjoin(figure_7_data_root, f'{session_name}_{pfc_name}_{dms_name}_interconnectivity_strength.npy'))
    else:
        ...
        np.save(pjoin(figure_7_data_root, f'{session_name}_{pfc_name}_{dms_name}_interconnectivity_strength.npy'), interconnectivity_strength)
    ...
    if plot and f'{session_name}_{pfc_name}_{dms_name}' in figure_7_selected and not isfile(pjoin(panel_abc_figure_root, f'{session_name}_{pfc_name}_{dms_name}.png')) and not check_probe_drift(interconnectivity_strength):
...
def figure_7_panel_dh(mono: bool = False, reset: bool = False):
    fig, axes = plt.subplots(1, 1, figsize=(5, 5))
    sig_rs_positive_percentage = []
    sig_rs_negative_percentage = []

    if reset:
        rmtree(figure_7_data_root)
        mkdir(figure_7_data_root)
    ...
    if mono:
        ...
        for mono_pair in tqdm.tqdm(dms_pfc_paths.iterrows()):
            ...
            p, r, _ = figure_7_panel_abc(session_name, pfc_name, dms_name, pfc_times, dms_times, cue_time, reward_proportion, reset=reset, plot=False)
    ...
    figure_7_panel_d_data = pd.DataFrame({'positive': sig_rs_positive_percentage, 'negative': sig_rs_negative_percentage})
    if mono:
        figure_7_panel_d_data.to_csv(pjoin(figure_data_root, 'figure_7_panel_dh_mono.csv'), index=False)
    else:
        figure_7_panel_d_data.to_csv(pjoin(figure_data_root, 'figure_7_panel_dh.csv'), index=False)
...
def figure_7_panel_ei(mono: bool = False, reset: bool = False):
    fig, axes = plt.subplots(1, 1, figsize=(5, 5))
    ...
    if mono:
        ...
        for mono_pair in tqdm.tqdm(dms_pfc_paths.iterrows()):
            ...
            # plot figure 6 poster panel ab
            p, r, overall_crosscor = figure_7_panel_abc(session_name, pfc_name, dms_name, pfc_times, dms_times, cue_time, reward_proportion, reset=reset, plot=False)
    ...
    figure_7_panel_ei_data = pd.DataFrame({'trial_lag': np.arange(-50, 51, 1), 'cross_correlation': overall_crosscors, 'cross_correlation_sem': overall_crosscors_sem})
    if mono:
        figure_7_panel_ei_data.to_csv(pjoin(figure_data_root, 'figure_7_panel_ei_data_mono.csv'), index=False)
    else:
        figure_7_panel_ei_data.to_csv(pjoin(figure_data_root, 'figure_7_panel_ei_data.csv'), index=False)
...
def figure_7_panel_fj(mono: bool = False, reset: bool = False):
    fig, axes = plt.subplots(1, 1, figsize=(5, 5))
    ...
    # save the data into a dataframe
    figure_7_panel_fj_data = pd.DataFrame({'trial_type': ['rewarded', 'non-rewarded'], 'strength': [rewarded_strength_mean, non_rewarded_strength_mean], 'sem': [rewarded_strength_sem, non_rewarded_strength_sem]})
    if mono:
        figure_7_panel_fj_data.to_csv(pjoin(figure_data_root, 'figure_7_panel_fj_data_mono.csv'), index=False)
    else:
        figure_7_panel_fj_data.to_csv(pjoin(figure_data_root, 'figure_7_panel_fj_data.csv'), index=False)
...
def figure_7_panel_gk(mono: bool = False, reset: bool = False):
    fig, axes = plt.subplots(1, 1, figsize=(5, 5))
    ...
    # save the data into a dataframe
    figure_7_panel_gk_data = pd.DataFrame({'trial_type': ['plateau', 'transition'], 'strength': [plateau_strength_mean, transition_strength_mean], 'sem': [plateau_strength_sem, transition_strength_sem]})
    if mono:
        figure_7_panel_gk_data.to_csv(pjoin(figure_data_root, 'figure_7_panel_gk_data_mono.csv'), index=False)
    else:
        figure_7_panel_gk_data.to_csv(pjoin(figure_data_root, 'figure_7_panel_gk_data.csv'), index=False)
...
def process_session_panel_ei(session, reset=False):
    ...
    for pair in session[3]:
        ...
        # plot figure 6 poster panel ab
        p, r, overall_crosscor = figure_7_panel_abc(session_name, pfc_name, dms_name, pfc_times, dms_times, cue_time, reward_proportion, reset=reset, plot=True)
